...
Version : 3.7 · Date : 28 juillet 2026 · Portée : améliorations livrées + backlog priorisé
...
**implémentées et testées** (suite passée à **264 tests / 724 assertions**, analyse
...
| **Sprint +27** ✅ | API / Mobile (Phase 5) | **Notifications & temps réel mobile** sous `/api/v1` : `GET /notifications` (pagination par curseur, compteur non-lus, projection stable), `POST /notifications/read`, et `GET /ws-ticket` (**ticket signé** réutilisant `RealtimeController` pour ouvrir la connexion WebSocket — messages/frappe/présence en direct) ; portées `notifications:read/write`. Flux vérifié en bout-en-bout ; **OpenAPI 1.11.0** — *livré* |
