...
Version du document : 3.9 · Date : 28 juillet 2026 · Statut : socle industrialisé (Sprints +1 → +25 livrés)
...
| Qualité | ✅ Livré | **257 tests PHPUnit** (unit + intégration MySQL/Redis) + **PHPStan niveau 5** + tests de charge, tous verts |
...
- [~] Applications natives (React Native / Flutter) réutilisant l'API REST — **socle API prêt** : authentification par **jeton porteur** stateless, révocable et à portée, versionnée `/api/v1` (`ApiToken`, `ApiAuthenticate`, endpoints auth/me/tokens) + **découverte, swipe & matches** + **messagerie** (conversations, historique déchiffré, envoi chiffré au repos, accusés de lecture) via services partagés web/API + OpenAPI `bearerAuth` — *Sprints +23/+24/+25*. ✅ *(apps natives elles-mêmes à développer)*
...
- [x] **Matching par apprentissage v1** : profil de goût appris du feedback implicite (âge, vérifié, intérêts likés) qui reclasse la découverte (`Services\Matching`, pur & testable) — *Sprint +15*. ✅ *(embeddings/ML avancé à venir)*
- [x] **Brise-glaces assistés** : suggestions d'accroche personnalisées (intérêts communs, ville, métier, objectif) via `Services\Icebreaker` (interface pluggable pour un LLM) + intégration au fil de discussion — *Sprint +14*. ✅ *(LLM réel à brancher)*
