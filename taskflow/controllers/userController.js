...
async function profilePayload(userId, viewerId) {
  const [users] = await pool.query(
    'SELECT id, name, email, bio, avatar, active_modules, profile_type FROM users WHERE id = ?',
    [userId]
  );
  const user = users[0];
  ...
  // Parse active_modules if it is a string (e.g. on older MySQL/MariaDB adapters)
  let parsedModules = user.active_modules;
  ...
  return {
    ...
    profile_type: user.profile_type || 'default',
...
// PUT /api/users/me - met à jour le nom, la bio, les modules actifs et le profil.
const updateMe = asyncHandler(async (req, res) => {
  const { name, bio, active_modules, profile_type } = req.body;
  const [[current]] = await pool.query('SELECT name, bio, active_modules, profile_type FROM users WHERE id = ?', [req.userId]);

  let activeModulesVal = current.active_modules;
  if (active_modules !== undefined) {
    activeModulesVal = Array.isArray(active_modules) ? JSON.stringify(active_modules) : active_modules;
  }

  await pool.query(
    'UPDATE users SET name = ?, bio = ?, active_modules = ?, profile_type = ? WHERE id = ?',
    [
      name !== undefined ? name : current.name,
      bio !== undefined ? bio : current.bio,
      activeModulesVal,
      profile_type !== undefined ? profile_type : current.profile_type,
      req.userId
    ]
  );
  res.json(await profilePayload(req.userId, req.userId));
